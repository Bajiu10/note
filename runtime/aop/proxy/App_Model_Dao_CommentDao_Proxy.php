...
class CommentDao
{
    ...
    public function read(ServerRequestInterface $request, int $id, int $pageSize = 5) : array
    {
        ...
        $fields = ['c.id', 'c.comment as comment', 'UNIX_TIMESTAMP(create_time) create_time', 'c.name', 'parent_id', 'count(f.user_id) hearts'];
        $hearts = Heart::where('comment_id', $request->ip())->column('user_id')->toArray();
        $comments = $this->query->table('comments', 'c')->leftJoin('hearts', 'f')->on('c.id', 'f.comment_id')->where('note_id', $id)->whereNull('parent_id')->group('c.id')->order(...$orders)->limit($pageSize)->offset(($page - 1) * $pageSize)->get($fields);
        $children = $this->query->table('comments', 'c')->leftJoin('hearts', 'f')->on('c.id', 'f.comment_id')->whereIn('parent_id', $comments->pluck('id')->toArray())->group('c.id')->order('hearts', 'DESC')->get($fields)->map(function ($item) use($hearts) {
