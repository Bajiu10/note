 ...
 */
#[Controller(prefix: '/')]
class NoteBaseController extends BaseController
{
    ...
    public function show($id) : ResponseInterface
    {
        if (!empty($note = $this->noteDao->findOne($id))) {
            Note::where('id', $id)->update(['hits' => new Expression('hits + 1')]);
    ...
    public function create() : ResponseInterface
    {
        ...
        try {
            $insertedId = $this->noteDao->createOne($this->session->get('user.id'), $this->request->post(['title', 'text', 'tags', 'sort', 'abstract', 'cid', 'thumb', 'permission'], ['tags' => '']));
        } catch (Exception $e) {
    ...
    public function edit($id) : ResponseInterface
    {
        if ($note = $this->noteDao->findOne($id, $this->session->get('user.id'))) {
            ...
            try {
                $this->noteDao->updateOne($id, $this->request->post(['title', 'sort', 'text', 'permission', 'tags', 'abstract', 'cid', 'thumb'], ['tags' => '', 'sort' => 0]));
