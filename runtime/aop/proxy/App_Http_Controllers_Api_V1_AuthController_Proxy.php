...
class AuthController extends ApiController
{
    ...
    public function register() : ResponseInterface
    {
        ...
        $validator = (new Validator())->make($data, ['username' => 'required', 'email' => 'email|required', 'password' => 'required', 'avatar' => 'required']);
        if ($validator->fails()) {
    ...
    public function token() : ResponseInterface
    {
        if ($user = $this->session->get('user')) {
            return $this->success(['token' => $this->jwt->encode(['id' => $user['id']]), 'user' => $user]);
        }
